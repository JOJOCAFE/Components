...
`examples/circuits/RV8GR_*` package as `Tested`; executable checks require each
...
1. Every RV8GR chip has explicit chip-level readiness, all five split-record
   ...
   evidence.
2. Every circuit package listed in `examples/circuits/README.md` exists, has tests,
...
5. Physical build notes do not claim speed beyond measured/source-backed
   evidence.
6. The next unproven item is visible in `examples/circuits/BACKLOG.md`.

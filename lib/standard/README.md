...
changes must bump that model version in `lib/standard/index.json`.
...
That shape is defined by `lib/standard/digital.schema.json` and applies to `74xx` and
...
Do not keep these in `lib/standard/`:

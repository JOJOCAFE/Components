...
```sh
PYTHONPATH=python python3 -B -m chiplib.cli run examples/circuits/nand.json > /tmp/nand.run.json
...
The easiest example is `examples/circuits/nand.json`.
...
```sh
PYTHONPATH=python python3 -B -m chiplib.cli probe examples/circuits/nand.json
```

Make a normalized copy:

```sh
PYTHONPATH=python python3 -B -m chiplib.cli export-json examples/circuits/nand.json -o /tmp/nand.normal.json
```

Export a netlist:

```sh
PYTHONPATH=python python3 -B -m chiplib.cli export-netlist examples/circuits/nand.json -o /tmp/nand.net.json
```

Try Verilog export:

```sh
PYTHONPATH=python python3 -B -m chiplib.cli export-verilog examples/circuits/nand.json -o /tmp/nand.verilog.json
PYTHONPATH=python python3 -B -m chiplib.cli export-verilog examples/circuits/nand.json --text -o /tmp/nand.v
```
...
```sh
PYTHONPATH=python python3 -B -m chiplib.cli circuit-faults examples/circuits/RV8GR_WholeSystemChipLevelVirtual/circuit.json > /tmp/circuit-faults.json
...
- `lib/standard/STUDENT_CATALOG.md`: student catalog fields.
- `lib/standard/COMPONENT_TEST_PROTOCOL.md`: serious chip/circuit test protocol.
- `examples/circuits/README.md`: RV8GR circuit packages.
